...
function buildCoordinatorInstructions(agent: ProjectBoardAgent, currentTools = true): string {
  return [
    ...
    currentTools ? 'For a failed check, use reopen_task with a repair reason (reopen completed dependent verification first), preserve the previous handoff, repair the affected work, then verify again. Do not replace completed history.' : 'If a completed task needs repair, ask the user to reopen the affected task on the board before continuing, or create a separate follow-up feature. Preserve previous handoffs.',
    'Use Codex native subagents when separate context or specialist work is useful. Include the selected profile instructions and complete task context when delegating because child agents begin with fresh context. The profile model and reasoningEffort are defaults: an explicit user or task request may override them for a new delegation without changing the saved profile. Apply only overrides supported by the native tool and selected model, follow its context/fork constraints, and do not claim unsupported settings were applied. A new override does not change an already running child.',
...
export class ProjectBoardService {
  ...
  private async startFeatureRun(featureId: string, continuation: boolean, allowWorkspaceWrite: boolean, kind: 'plan' | 'execute' = 'execute', queue?: ProjectBoardQueue): Promise<ProjectBoardSnapshot> {
    ...
    if (queue && queue.status !== 'running') throw new Error('The queue was paused before this feature started.')
