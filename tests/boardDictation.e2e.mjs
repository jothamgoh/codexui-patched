...
try {
  ...
  await touch.getByRole('status').filter({ hasText: /^Recording$/ }).waitFor()
  assert.equal(await touch.getByText('Stop when you’re ready.', { exact: false }).count(), 0)
  const cancel = touch.getByRole('button', { name: 'Cancel dictation for Goal or plan', exact: true })
  ...
  await cancel.click()
  assert.equal(await touch.getByRole('status').filter({ hasText: /^Recording$/ }).count(), 0)
  assert.equal(await touch.getByRole('textbox', { name: 'Goal or plan', exact: true }).inputValue(), 'One\nTwo')
  await touch.close()
