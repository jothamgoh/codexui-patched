...
test('persisted follow-ups preserve Done and reopen a real repair task only after capacity and reason checks', async (t) => {
  const fixture = await createFixture(t)
  const { store } = fixture
  const { board, feature } = await createBoardAndFeature(store)
  const { run } = await store.startRun(feature.id, 'builtin-lead', 'execute')
  await store.setRunThread(run.id, 'completed-chat', 3)
  const planned = await store.replacePlan(feature.id, { summary: 'Implement.', tasks: [planTask({})] }, run.id)
  const task = planned.cards.find((card) => card.parentCardId === feature.id)
  await store.updateTaskFromAgent(feature.id, task.id, 'start', {}, run.id)
  await store.updateTaskFromAgent(feature.id, task.id, 'complete', { summary: 'Original handoff.' }, run.id)
  await store.finishFeature(feature.id, 'Original result.', run.id)
  await store.completeRun(run.id, 'Complete.')
  const completed = await store.read()
  const followup = await store.startRun(feature.id, 'builtin-lead', 'follow_up')
  await store.setRunThread(followup.run.id, 'completed-chat', 3)
  const loaded = await fixture.reopen().read()
  assert.equal(loaded.runs[0].kind, 'follow_up')
  assert.equal(loaded.cards.find((card) => card.id === feature.id).toolSchemaVersion, 3)
  assert.deepEqual(loaded.cards, completed.cards)
  const sibling = (await store.createCard({ boardId: board.id, title: 'Other work' })).cards.find((card) => card.title === 'Other work')
  const siblingRun = await store.startRun(sibling.id, 'builtin-lead', 'execute')
  await assert.rejects(store.reopenFollowUp(followup.run.id, 'Change requested.'), /Another feature is running/u)
  await store.failRun(siblingRun.run.id, 'Stopped.', 'interrupted')
  await assert.rejects(store.reopenFollowUp(followup.run.id, ''), /repair reason/u)
  assert.deepEqual((await store.read()).cards.find((card) => card.id === feature.id), completed.cards.find((card) => card.id === feature.id))
  const repaired = await store.reopenFollowUp(followup.run.id, 'Fix the newly reported behavior.')
  assert.equal(repaired.runs.find((entry) => entry.id === followup.run.id).kind, 'execute')
  assert.equal(repaired.cards.find((card) => card.id === feature.id).status, 'working')
  assert.equal(repaired.cards.find((card) => card.id === task.id).status, 'backlog')
  assert.equal(repaired.cards.find((card) => card.id === task.id).summary, 'Original handoff.')
  await assert.rejects(store.finishFeature(feature.id, 'Cannot reuse old checks.', followup.run.id), /All required tasks/u)
})
