...
try {
  ...
  assert.equal(planRequest.team.model, 'build-model')
  assert.equal(planRequest.team.reasoningEffort, 'high')
  ...
  assert.equal(planRequest.team.model, '')
  assert.equal(planRequest.team.reasoningEffort, '')
  ...
  async function checkBoardTeam(target, touch) {
    ...
    await feature.getByText('Using build-model · Extra high reasoning.', { exact: true }).waitFor()
